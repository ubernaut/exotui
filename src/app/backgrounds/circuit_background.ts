...
export interface ShellCircuitFieldOptions {
...
export interface ShellCircuitChipSnapshot {
  /** Stable gate identity, as referenced by `ShellCircuitTraceSnapshot`. */
  readonly id: number;
  readonly x: number;
  readonly y: number;
...
export interface ShellCircuitRailSnapshot {
  readonly x: number;
  readonly y: number;
  readonly label: string;
}
...
export interface ShellCircuitOscillatorSnapshot {
  readonly x: number;
  readonly y: number;
  readonly label: string;
...
export interface ShellCircuitTraceSnapshot {
...
export interface ShellCircuitLedSnapshot {
  readonly id: number;
  readonly x: number;
  readonly y: number;
...
export interface ShellCircuitInspection {
  readonly bounds?: Rectangle;
  readonly chips: readonly ShellCircuitChipSnapshot[];
  readonly traces: readonly ShellCircuitTraceSnapshot[];
  ...
  readonly power?: ShellCircuitRailSnapshot;
  /** The ground rail node, when placed. */
  readonly ground?: ShellCircuitRailSnapshot;
  /** Free-running signal generators placed on the board. */
  readonly oscillators: readonly ShellCircuitOscillatorSnapshot[];
  /** The indicator lamp array across the top of the board. */
  readonly leds: readonly ShellCircuitLedSnapshot[];
...
interface CircuitPointer extends ShellBackgroundPoint {
 ...
 */
export class ShellCircuitField implements ShellAnimatedBackground {
  #randomState: number;
  readonly #density: number;
  #bounds?: Rectangle;
  #pointer?: CircuitPointer;
  #activePointer?: ShellBackgroundPoint;
  #lastFrameAt?: number;
  #chips: CircuitChip[] = [];
  #traces: CircuitTrace[] = [];
  ...
  constructor(options: ShellCircuitFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const traceBase = mixShellRgb(theme.border, theme.background, 0.35);
    const viaColor = mixShellRgb(theme.border, theme.text, 0.15);
    const chipBorder = mixShellRgb(theme.border, theme.text, 0.25);
    const chipBody = mixShellRgb(theme.surfaceStrong, theme.text, 0.2);
    const labelColor = mixShellRgb(theme.muted, theme.text, 0.1);
    const pulseHead = mixShellRgb(theme.accent, theme.text, 0.2);
    const pulseTrail = mixShellRgb(theme.accent, traceBase, 0.5);
    const highlight = mixShellRgb(traceBase, theme.accent, 0.45);
    const activeTapBase = mixShellRgb(traceBase, theme.accent, ACTIVE_TAP_BASE_MIX);
    const activeTapVia = mixShellRgb(viaColor, theme.accent, 0.5);
    const activeTapHead = mixShellRgb(pulseHead, theme.text, 0.35);
    const activeTapTrail = mixShellRgb(pulseTrail, theme.accent, 0.5);
    ...
    const liveTrace = mixShellRgb(traceBase, theme.accent, 0.55);
    const idleTrace = mixShellRgb(traceBase, theme.background, 0.35);
    const liveChipBody = mixShellRgb(chipBody, theme.accent, 0.5);
    const liveChipBorder = mixShellRgb(chipBorder, theme.accent, 0.55);
    const liveLabel = mixShellRgb(labelColor, theme.text, 0.6);
    // A clicked gate lights its own wiring so a single net can be followed
    // across a crowded board.
    const selectedTrace = mixShellRgb(theme.warning, theme.text, 0.25);
    const selectedBorder = mixShellRgb(theme.warning, theme.text, 0.45);
    ...
    this.#paintRail(this.#power, theme, mixShellRgb(theme.warning, theme.text, 0.3), true, width, height);
    this.#paintRail(this.#ground, theme, mixShellRgb(theme.muted, theme.background, 0.2), false, width, height);
    ...
    const oscHigh = mixShellRgb(theme.accent, theme.text, 0.4);
    const oscLow = mixShellRgb(theme.accent, theme.background, 0.35);
    ...
    const ledOn = mixShellRgb(theme.success, theme.text, 0.35);
    const ledOff = mixShellRgb(theme.muted, theme.background, 0.55);
  ...
  inspect(): ShellCircuitInspection {
  ...
  #syncObstacles(options: ShellBackgroundAdvanceOptions, bounds: Rectangle): boolean {
