...
export interface ShellFireInspection {
...
export interface ShellFireFieldOptions {
  readonly seed?: number;
  /** Scales the ember source; 1 is a lively blaze, lower is a smoulder. */
  readonly intensity?: number;
}

interface FirePointer extends ShellBackgroundPoint {
 ...
 */
export class ShellFireField implements ShellAnimatedBackground {
  readonly #intensity: number;
  #randomState: number;
  #bounds?: Rectangle;
  #width = 0;
  ...
  constructor(options: ShellFireFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const ember = mixShellRgb(theme.background, theme.danger, 0.85);
    const flame = theme.warning;
    const core = mixShellRgb(theme.warning, theme.text, 0.7);
  ...
  #syncObstacles(options: ShellBackgroundAdvanceOptions, bounds: Rectangle): void {
...
function fireColor(heat: number, ember: ShellRgb, flame: ShellRgb, core: ShellRgb): ShellRgb {
  if (heat < 0.55) return mixShellRgb(ember, flame, heat / 0.55);
  if (heat < 0.82) return mixShellRgb(flame, core, (heat - 0.55) / 0.27);
  return core;
}
