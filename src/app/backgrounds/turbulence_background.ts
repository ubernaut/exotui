...
import { shellGpuDevice } from "./gpu_device.ts";
...
async function createTurbulenceGpu(width: number, height: number): Promise<TurbulenceGpuContext | null> {
  try {
    ...
    const device = await shellGpuDevice();
...
interface TurbulencePointer extends ShellBackgroundPoint {
  readonly updatedAt: number;
}

/** Construction options. */
export interface ShellTurbulenceFieldOptions {
 ...
 */
export class ShellTurbulenceField implements ShellAnimatedBackground {
  #bounds?: Rectangle;
  ...
  constructor(_options: ShellTurbulenceFieldOptions = {}) {}
  ...
  rasterizeCells(
    ...
    const cwColor: ShellRgb = mixShellRgb(theme.accent, theme.text, 0.3);
    const ccwColor: ShellRgb = mixShellRgb(theme.muted, theme.text, 0.3);
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        const foreground = mixShellRgb(theme.background, hue, intensity);
