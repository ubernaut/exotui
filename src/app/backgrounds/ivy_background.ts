...
export type ShellIvyOrnament = "none" | "leaf" | "flower" | "fruit";

/** One section of a strand exposed for deterministic tests. */
export interface ShellIvyCellSnapshot {
  readonly x: number;
  readonly y: number;
  /** How long this section has existed, in milliseconds. */
  readonly ageMs: number;
  readonly ornament: ShellIvyOrnament;
...
export interface ShellIvyStrandSnapshot {
  readonly cells: readonly ShellIvyCellSnapshot[];
  readonly ageMs: number;
  readonly growing: boolean;
}

/** Serializable ivy inspection for tests and diagnostics. */
export interface ShellIvyInspection {
  readonly strands: readonly ShellIvyStrandSnapshot[];
...
export interface ShellIvyFieldOptions {
...
interface IvyPointer extends ShellBackgroundPoint {
 ...
 */
export class ShellIvyField implements ShellAnimatedBackground {
  readonly #density: number;
  #randomState: number;
  #bounds?: Rectangle;
  #strands: IvyStrand[] = [];
  ...
  constructor(options: ShellIvyFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const vineDeep = mixShellRgb(theme.background, theme.success, 0.45);
    const vineLit = mixShellRgb(theme.success, theme.text, 0.3);
    const flowerPalette: readonly ShellRgb[] = [
      theme.accent,
      theme.warning,
      theme.danger,
      mixShellRgb(theme.accent, theme.text, 0.4),
    ];
    ...
    for (const strand of this.#strands) {
      for (const cell of strand.cells) {
        ...
        const stalkShade = mixShellRgb(vineDeep, vineLit, 0.2 + maturity * 0.6);
        ...
        if (cell.ageMs >= THICKEN_AGE_MS) {
          ...
          if (edgeX >= 0 && edgeX < width && edgeY >= 0 && edgeY < height && !this.#cells[edgeY]![edgeX]) {
            this.#cells[edgeY]![edgeX] = {
              ...
              foreground: mixShellRgb(vineDeep, stalkShade, 0.55),
            };
          }
        }
        ...
        if (cell.ornament === "leaf") {
          this.#cells[cell.y]![cell.x] = {
            ...
            foreground: mixShellRgb(stalkShade, theme.success, 0.5),
          };
          ...
          this.#cells[cell.y]![cell.x] = {
            ...
            foreground: mixShellRgb(vineLit, petal, 0.3 + openness * 0.7),
          ...
          this.#cells[cell.y]![cell.x] = {
            ...
            foreground: ripe ? berry : mixShellRgb(stalkShade, berry, 0.4),
            ...(ripe ? { bold: true } : {}),
          };
        }
      }
    ...
    for (const particle of this.#confetti) {
      ...
      this.#cells[y]![x] = {
        ...
        foreground: mixShellRgb(spark, vineDeep, life * 0.7),
        ...(life < 0.5 ? { bold: true } : {}),
      };
    }
   ...
   */
  pick(column: number, row: number, now: number = performance.now()): boolean {
