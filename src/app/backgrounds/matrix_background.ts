...
export interface ShellMatrixRainFieldOptions {
...
export interface ShellMatrixRainDropSnapshot {
...
export interface ShellMatrixRainInspection {
  readonly bounds?: Rectangle;
  readonly drops: readonly ShellMatrixRainDropSnapshot[];
}
...
interface MatrixPointer extends ShellBackgroundPoint {
 ...
 */
export class ShellMatrixRainField implements ShellAnimatedBackground {
  #randomState: number;
  readonly #density: number;
  #bounds?: Rectangle;
  ...
  constructor(options: ShellMatrixRainFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const headBase = mixShellRgb(theme.text, theme.accent, 0.35);
    const obstacle = this.#activeObstacle;
    for (const drop of this.#drops) {
      const { column } = drop;
      ...
      for (let offset = 0; offset <= drop.tail; offset += 1) {
        ...
        if (offset === 0) {
          const foreground = drop.boost > 0 ? mixShellRgb(headBase, theme.text, 0.45 * drop.boost) : headBase;
          this.#cells[row]![column] = { char: glyph, foreground, bold: true };
          continue;
        }
        const fade = (0.18 + 0.8 * (offset / drop.tail)) * (1 - 0.3 * drop.boost);
        this.#cells[row]![column] = { char: glyph, foreground: mixShellRgb(theme.accent, theme.background, fade) };
      }
    }
    ...
    for (const splash of this.#splashes) {
      ...
      for (const arc of SPLASH_ARC_PARTICLES) {
        ...
        const foreground = mixShellRgb(theme.background, headBase, alpha);
    ...
    const gutterColor = mixShellRgb(theme.accent, theme.background, 0.25);
    for (const gutter of this.#gutterDrops) {
      if (!gutter.falling) {
        ...
        for (let t = 0; t <= GUTTER_SLIDE_TAIL; t += 1) {
          ...
          const fg = mixShellRgb(gutterColor, theme.background, fade);
  ...
  inspect(): ShellMatrixRainInspection {
    return {
