...
- **Right-click context item.** `exomuxStartMenuItems(controller)` (in `app.ts`) slots a **"Favorite ☐/☑"** item into
  the start menu just below "Settings" whenever the active background is a butterchurn one and a preset is showing (a
  checked box when the showing preset is already a favorite). The preset showing at open time is captured in
  `controller.startMenuPreset` so paint, hit-test, and keyboard all agree on the item list. The menu opens on
  right-click over the desktop (or the start button) as before; the item just extends it. (A "Background settings" item
  was tried and then dropped at the user's request — background config is still reached from the Settings UI.)
